...
impl SecurityFramework {
    ...
    /// 验证能力令牌
    pub fn verify_capability(&self, token: &CapabilityToken, resource: &str, permission: &Permission) -> bool {
        ...
        if !self.match_resource(&token.resource, resource) {
            warn!("资源不匹配: 令牌允许访问 {}，请求访问 {}", token.resource, resource);
            return false;
        }
        
        // 检查权限
        if !self.has_permission(token, permission) {
            warn!("权限不足: 令牌 {:?} 缺少访问 {} 资源的 {:?} 权限", token.id, resource, permission);
            return false;
        }
...
impl CapabilityManager {
    ...
    pub fn issue_token(
        ...
        debug!("正在颁发能力令牌，资源: {}, 权限: {:?}, 接收者: {}", resource, permissions, subject);
