...
pub fn deserialize(data: &[u8]) -> Result<Message, String> {
    ...
    let msg_type = match data[offset] {
        ...
        0x01 => {
            ...
            // 反序列化数据类型
            let data_type =
                String::from_utf8_lossy(&data[offset..offset + data_type_len]).to_string();
            ...
            message::MessageType::Data(message::DataMessage { data_type, data })
        }
        0x02 => {
            offset += 1;
            // 反序列化事件类型长度
            ...
            // 反序列化事件类型
            let event_type =
                String::from_utf8_lossy(&data[offset..offset + event_type_len]).to_string();
