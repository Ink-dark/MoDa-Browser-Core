...
impl SecurityManager {
    ...
    pub fn init(&mut self) -> Result<(), String> {
        // 初始化默认加密密钥
        self.encryption_keys.insert("default".to_string(), Self::generate_default_key());
        Ok(())
    }
    ...
    pub fn encrypt_message(&self, msg: Vec<u8>) -> Result<Vec<u8>, String> {
        // 获取默认加密密钥
        let key = self.encryption_keys.get("default").ok_or("No encryption key found".to_string())?;
        
        // 这里使用简单的XOR加密，实际应用中应该使用更安全的加密算法
        let encrypted = msg.iter()
            .zip(key.iter().cycle())
            .map(|(m, k)| m ^ k)
            .collect();
    ...
    pub fn decrypt_message(&self, encrypted_msg: Vec<u8>) -> Result<Vec<u8>, String> {
        ...
        let decrypted = encrypted_msg.iter()
            .zip(key.iter().cycle())
            .map(|(m, k)| m ^ k)
            .collect();
