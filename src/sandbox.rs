...
impl Sandbox {
    ...
    pub fn start(&self) -> Result<(), String> {
        ...
        if !matches!(*state, SandboxState::Created) {
            return Err(format!("沙箱 {} 已处于 {:?} 状态，无法启动", self.config.name, *state));
        }
    ...
    pub fn pause(&self) -> Result<(), String> {
        ...
        if !matches!(*state, SandboxState::Running) {
            return Err(format!("沙箱 {} 未处于运行状态，无法暂停", self.config.name));
        }
    ...
    pub fn resume(&self) -> Result<(), String> {
        ...
        if !matches!(*state, SandboxState::Paused) {
            return Err(format!("沙箱 {} 未处于暂停状态，无法恢复", self.config.name));
        }
    ...
    pub fn terminate(&self) -> Result<(), String> {
        ...
        if let Some(pid) = pid {
            ...
            match self.terminate_process(pid) {
                ...
                Err(e) => {
                    error!("终止沙箱进程失败: {}, PID: {}, 错误: {}", self.config.name, pid, e);
                    Err(e)
                },
            }
        } else {
    ...
    fn terminate_process(&self, pid: u32) -> Result<(), String> {
        ...
        use windows_sys::Win32::Foundation::{HANDLE, BOOL, ERROR_ACCESS_DENIED, ERROR_INVALID_PARAMETER};
